...
if (isset($_POST['beddingsize']) && is_array($_POST['beddingsize'])) {
    $selectedsize = $_POST['beddingsize'];
    
    // Join the selected sizes into a comma-separated string
    $beddingSize = implode(',', $selectedsize);
} else {
    $beddingSize = ''; // Default value if no sizes were selected
}


$sql="INSERT INTO `product_details`(`mainCat`,`sub_categorey`,`brand`,`Productname`,`productPrice`,`discountpercentage`,`productQuantity`,`beddingsize`,`colorfamily`,`warrantytype`,`warrantyperiod`,`coreconstruction`,`patterntype`,`linenfabric`,`featuretype`,`productInfo`,`productDes`,`productImage1`,`productImage2`,`productImage3`,`materialtype`,`shipping_cost`,`stockStatus`,`trending`,`product_type`)
VALUES ('$mainCat','$bedcategorey','$brand','$Productname',$productPrice,$discountpercentage,$productQuantity,'$beddingSize','$colorFamily','$warrantytype','$warrantyperiod','$coreconstruction','$patterntype','$linenfabric','$featuretype','$productInfo','$productDes','$image1','$image2','$image3','$materialtype',$shipping_cost,'$stockStatus','$trending','$product_type')"; 
...
<body>
    ...
    <main class="main-wrapper">
        ...
        <form class="row g-3" enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']?>">
            ...
            <div class="col-md-4">
                <label for="inputbedcategorey" class="form-label">Bed Category</label>
                <select class="form-select" aria-label="Default select example" name="bedcategorey">
                    <?php
$getCategory="SELECT distinct(category_name) FROM furniture_beds_category";
...
while($rowSub2 = $resultSubCategory2->fetch_array()){
?>
                    <option value="<?php echo $rowSub2['core_construction_ype'] ?>">
                        <?php echo $rowSub2['core_construction_ype'] ?>
                    </option>
                    <?php
}
        ?>
...
?>
        ...
        ?>
            ...
            <div class="col-md-10">
                <label for="inputbeddingsize" class="form-label">Bedding Size :</label>
                <?php
$getCategory="SELECT distinct(bed_size) FROM furniture_bedding_size";
$resultSubCategory1=$conn->query($getCategory);
while($rowSub1 = $resultSubCategory1->fetch_array()){
?>

                <input type="checkbox" name="beddingsize[]" value="<?php echo $rowSub1['bed_size'] ?>">
                <?php echo $rowSub1['bed_size'] ?>
...
?>
            ...
            <div class="col-3">
                <input type="submit" class="btn btn-primary" value="Add Product" required name="InsertProduct">
            </div>
            <div class="col-6">
